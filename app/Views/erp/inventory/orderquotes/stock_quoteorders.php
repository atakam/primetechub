...
if($user_info['user_type'] == 'staff'){
	$get_invoices = $OrderquotesModel->where('company_id',$user_info['company_id'])->orderBy('quote_id', 'ASC')->paginate(8);
	$count_invoices = $OrderquotesModel->where('company_id',$user_info['company_id'])->orderBy('quote_id', 'ASC')->countAllResults();
	$pager = $OrderquotesModel->pager;
	$company_id = $user_info['company_id'];
} else {
	$get_invoices = $OrderquotesModel->where('company_id',$usession['sup_user_id'])->orderBy('quote_id', 'ASC')->paginate(8);
	$count_invoices = $OrderquotesModel->where('company_id',$usession['sup_user_id'])->orderBy('quote_id', 'ASC')->countAllResults();
	$company_id = $usession['sup_user_id'];
	$pager = $OrderquotesModel->pager;
}
$unpaid = $OrderquotesModel->where('company_id',$company_id)->where('status', 0)->countAllResults();
$paid = $OrderquotesModel->where('company_id',$company_id)->where('status', 1)->countAllResults();
/*
...
<div id="smartwizard-2" class="border-bottom smartwizard-example sw-main sw-theme-default mt-2">
  <ul class="nav nav-tabs step-anchor">
    ...
    <li class="nav-item active"> <a href="<?= site_url('erp/order-quotes');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon feather icon-calendar"></span>
    ...
    <li class="nav-item clickable"> <a href="<?= site_url('erp/create-quote');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon fas fa-calendar-plus"></span>
...
<div class="row">
  ...
  <div class="col-xl-12 col-lg-12 filter-bar invoice-list">
    <nav class="navbar m-b-30 p-10">
      ...
      <?php if(in_array('quote_order2',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
      <div class="nav-item nav-grid f-view"> <a href="<?= site_url().'erp/create-quote';?>" class="btn waves-effect waves-light btn-primary btn-sm m-0"> <i data-feather="plus"></i>
        <?= lang('Inventory.xin_create_quote');?>
        </a> </div>
      <?php } ?>
    </nav>
    <div class="row">
      ...
      <?php
		$invoice_date = set_date_format($r['quote_date']);
		$invoice_due_date = set_date_format($r['quote_due_date']);
		// status
		if($r['status']==0){
			$status = '<span class="badge badge-light-primary">'.lang('Inventory.xin_quoted').'</span>';
		} else if($r['status']==1){
        ...
        <div class="card card-border-c-blue">
          <div class="card-body">
            ...
            <div class="m-t-30">
              ...
              <div class="task-board m-0 float-right"> <a href="<?= site_url().'erp/quote-detail/'.uencode($r['quote_id']);?>" class="btn btn-primary"><i class="fas fa-eye m-0"></i></a>
                <div class="dropdown-secondary dropdown d-inline">
                  ...
                  <div class="dropdown-menu dropdown-menu-right" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut"> <a class="dropdown-item" href="<?= site_url().'erp/print-quote/'.uencode($r['quote_id']);?>"><i class="fas fa-download mr-2"></i>
                    <?= lang('Inventory.xin_download_quote');?>
                    </a>
                    <div class="dropdown-divider"></div>
                    <?php if($r['status']==0){ ?>
						<?php if(in_array('invoice4',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                            <a class="dropdown-item" href="<?= site_url().'erp/edit-quote/'.uencode($r['quote_id']);?>"><i class="fas fa-edit mr-2"></i>
                            <?= lang('Inventory.xin_edit_quote');?>
                            </a>
                        <?php } ?>
                    <?php } ?>
                    <?php if(in_array('invoice5',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                    <a href="#!" class="dropdown-item delete" data-toggle="modal" data-target=".delete-modal" data-record-id="<?= uencode($r['quote_id']);?>"><i class="feather icon-trash-2"></i>
                    <?= lang('Inventory.xin_quote_quote');?>
                    </a>
                    <?php } ?>
                    </div>
                </div>
