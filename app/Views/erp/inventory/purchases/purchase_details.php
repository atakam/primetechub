...
$purchase_id = udecode($segment_id);
$xin_system = erp_company_settings();
$inv_company_info = $UsersModel->where('user_id', $xin_system['company_id'])->first();

$result = $PurchasesModel->where('purchase_id', $purchase_id)->first();
$supplier_info = $SuppliersModel->where('supplier_id', $result['supplier_id'])->first();
if($supplier_info){
	$supplier_contact = $supplier_info['supplier_name'];
	$address_1 = $supplier_info['address_1'].' '.$supplier_info['address_2'];
	$cemail = $supplier_info['email'];
	$ccontact_number = $supplier_info['contact_number'];
	$csz = $supplier_info['city'].', '.$supplier_info['state'].' '.$supplier_info['zipcode'];
} else {
	$csz = '--';
	$cemail = '--';
	$address_1 = '--';
...
$invoice_items = $PurchaseitemsModel->where('purchase_id', $purchase_id)->findAll();
      ...
      <div>
        <div class="card" id="printTable">
          <div class="card-header">
            <h5>
              <?= lang('Inventory.xin_view_purchase').' #'.$result['purchase_number'];?>
            </h5>
            <div class="card-header-right"> <a href="<?= site_url('erp/print-purchase/').$segment_id;?>" target="_blank" class="collapsed btn btn-sm waves-effect waves-light btn-success m-0">
              <?= lang('Invoices.xin_print_download_invoice');?>
              </a>
              <?php if($user_info['user_type'] != 'customer'){ ?>
				  <?php if($result['status'] == 0) { ?>
					  <?php if(in_array('purchases5',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                      ...
                      <a href="<?= site_url('erp/edit-purchase/').$segment_id;?>" class="collapsed btn btn-sm waves-effect waves-light btn-primary m-0">
                      <?= lang('Main.xin_edit');?>
                      </a>
                      <?php } ?>
                  <?php } ?>
          ...
          <div class="card-body">
            ...
            <div class="row invoive-info d-print-inline-flex">
              <div class="col-sm-4 invoice-client-info">
                <h6>
                  <?= lang('Inventory.xin_supplier_info');?>
                  :</h6>
                <h6 class="m-0">
                  <?= $supplier_contact;?>
                </h6>
              ...
              <div class="col-sm-4">
                <h6>
                  <?= lang('Inventory.xin_purchase_info');?>
                  :</h6>
              ...
              <div class="col-sm-4">
                <h6 class="m-b-20">
                  <?= lang('Inventory.xin_purchase_number');?>
                  <span>#
