...
if($user_info['user_type'] == 'staff'){
	...
	$get_invoice = $OrdersModel->where('company_id',$user_info['company_id'])->where('order_id', $invoice_id)->first();
} else {
	...
	$get_invoice = $OrdersModel->where('company_id',$usession['sup_user_id'])->where('order_id', $invoice_id)->first();
}
$xin_system = erp_company_settings();
$invoice_items = $OrderitemsModel->where('order_id', $invoice_id)->findAll();
?>
<?php
// Create Invoice Page
?>
<?php $get_animate = '';?>
<?php if(in_array('sales_order1',staff_role_resource()) || in_array('sales_order2',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
<div id="smartwizard-2" class="border-bottom smartwizard-example sw-main sw-theme-default mt-2">
  <ul class="nav nav-tabs step-anchor">
    <?php if(in_array('sales_order1',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
    <li class="nav-item clickable"> <a href="<?= site_url('erp/stock-orders');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon feather icon-calendar"></span>
      <?= lang('Inventory.xin_sales_order');?>
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header with-elements"> <span class="card-header-title mr-2"><strong>
        <?= lang('Inventory.xin_edit_order');?>
        </strong></span> </div>
      <div class="card-body" aria-expanded="true" style="">
        <div class="row m-b-1">
          <div class="col-md-12">
            <?php $attributes = array('name' => 'edit_order', 'id' => 'xin-form', 'autocomplete' => 'off', 'class' => 'form');?>
            <?php $hidden = array('token' => $segment_id);?>
            <?php echo form_open('erp/orders/update_order', $attributes, $hidden);?>
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                      <label for="order_number">
                        <?= lang('Inventory.xin_order_number');?> <span class="text-danger">*</span>
                      </label>
                      <div class="input-group">
                        <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-file-invoice"></i></span></div>
                        <input class="form-control" placeholder="<?= lang('Inventory.xin_order_number');?>" name="order_number" type="text" value="<?= $get_invoice['order_number'];?>">
                      </div>
                    </div>
                  </div>
                  <div class="col-md-3">
                    ...
                    <div class="form-group">
                          ...
                          <div data-repeater-item="">
                            <div class="row item-row">
                              <div class="form-group mb-1 col-sm-12 col-md-5">
                                ...
                                <br>
                                <select class="form-control item_name" name="eitem_name[<?php echo $item['order_item_id'];?>]" data-plugin="select_hrm" data-placeholder="<?= lang('Inventory.xin_choose_item');?>">
                                <option value=""></option>
                              ...
                              <div class="form-group mb-1 col-sm-12 col-md-2">
                                ...
                                <br>
                                <input type="text" class="form-control qty_hrs" name="eqty_hrs[<?php echo $item['order_item_id'];?>]" id="qty_hrs" value="<?= $item['item_qty'];?>">
                              </div>
                              <div class="skin skin-flat form-group mb-1 col-sm-12 col-md-2">
                                ...
                                <br>
                                <input class="form-control unit_price" type="text" name="eunit_price[<?php echo $item['order_item_id'];?>]" value="<?= $item['item_unit_price'];?>" id="unit_price" />
                              </div>
                              <div class="form-group mb-1 col-sm-12 col-md-2">
                                <label for="profession">
                                  <?= lang('Invoices.xin_subtotal');?>
                                </label>
                                <input type="text" class="form-control sub-total-item" readonly="readonly" name="esub_total_item[<?php echo $item['order_item_id'];?>]" value="<?= $item['item_sub_total'];?>" />
                                <!-- <br>-->
                              ...
                              <div class="form-group col-sm-12 col-md-1 text-xs-center mt-2">
                                ...
                                <?php else:?>
                                <button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light remove-invoice-item-ol" data-repeater-delete="" data-record-id="<?= uencode($item['order_item_id']);?>"> <span class="fa fa-trash"></span></button>
                                <?php endif;?>
                              </div>
                            </div>
                          </div>
                      ...
                      <div class="row">
                        <div class="col-lg-12">
                          ...
                          <textarea name="invoice_note" class="form-control"><?= $get_invoice['invoice_note'];?></textarea>
                        </div>
                      </div>
                    </div>
                  </div>
