...
if($user_info['user_type'] == 'staff'){
	$get_invoices = $OrdersModel->where('company_id',$user_info['company_id'])->orderBy('order_id', 'ASC')->paginate(8);
	...
	$get_invoices = $OrdersModel->where('company_id',$usession['sup_user_id'])->orderBy('order_id', 'ASC')->paginate(8);
	$count_invoices = $OrdersModel->where('company_id',$usession['sup_user_id'])->orderBy('order_id', 'ASC')->countAllResults();
	$company_id = $usession['sup_user_id'];
	$pager = $OrdersModel->pager;
}
$unpaid = $OrdersModel->where('company_id',$company_id)->where('status', 0)->countAllResults();
$paid = $OrdersModel->where('company_id',$company_id)->where('status', 1)->countAllResults();
/*
...
<div id="smartwizard-2" class="border-bottom smartwizard-example sw-main sw-theme-default mt-2">
  <ul class="nav nav-tabs step-anchor">
    ...
    <li class="nav-item active"> <a href="<?= site_url('erp/stock-orders');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon feather icon-calendar"></span>
        ...
        <div class="card card-border-c-blue">
          <div class="card-body">
            ...
            <div class="m-t-30">
              ...
              <div class="task-board m-0 float-right"> <a href="<?= site_url().'erp/order-detail/'.uencode($r['order_id']);?>" class="btn btn-primary"><i class="fas fa-eye m-0"></i></a>
                <div class="dropdown-secondary dropdown d-inline">
                  ...
                  <div class="dropdown-menu dropdown-menu-right" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut"> <a class="dropdown-item" href="<?= site_url().'erp/print-order/'.uencode($r['order_id']);?>"><i class="fas fa-download mr-2"></i>
                    <?= lang('Invoices.xin_download_invoice');?>
                    </a>
                    <div class="dropdown-divider"></div>
                    <?php if($r['status']==0){ ?>
						<?php if(in_array('invoice4',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                            <a class="dropdown-item" href="<?= site_url().'erp/edit-order/'.uencode($r['order_id']);?>"><i class="fas fa-edit mr-2"></i>
                            <?= lang('Invoices.xin_edit_invoice');?>
                            </a>
                        <?php } ?>
                    <?php } ?>
                    <?php if(in_array('invoice5',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                    <a href="#!" class="dropdown-item delete" data-toggle="modal" data-target=".delete-modal" data-record-id="<?= uencode($r['order_id']);?>"><i class="feather icon-trash-2"></i>
