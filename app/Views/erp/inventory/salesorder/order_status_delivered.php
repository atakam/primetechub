...
if($user_info['user_type'] == 'staff'){
	$get_invoices = $OrdersModel->where('company_id',$user_info['company_id'])->where('status',4)->orderBy('order_id', 'ASC')->paginate(8);
	...
	$get_invoices = $OrdersModel->where('company_id',$usession['sup_user_id'])->where('status',4)->orderBy('order_id', 'ASC')->paginate(8);
	$count_invoices = $OrdersModel->where('company_id',$usession['sup_user_id'])->orderBy('order_id', 'ASC')->countAllResults();
	$company_id = $usession['sup_user_id'];
	$pager = $OrdersModel->pager;
}
?>
<?php if(in_array('delivered_orders',staff_role_resource()) || in_array('sales_order2',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
<div id="smartwizard-2" class="border-bottom smartwizard-example sw-main sw-theme-default mt-2">
  <ul class="nav nav-tabs step-anchor">
    <?php if(in_array('delivered_orders',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
    <li class="nav-item active"> <a href="<?= site_url('erp/delivered-orders');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon feather icon-calendar"></span>
      <?= lang('Inventory.xin_delivered_orders');?>
