...
$company_info = $UsersModel->where('user_id', $result['customer_id'])->where('user_type', 'customer')->first();
if($company_info){
      ...
      <div>
        <div class="card" id="printTable">
          <div class="card-header">
            <h5>
              <?= lang('Invoices.xin_view_invoice').' #'.$result['order_number'];?>
            </h5>
            <div class="card-header-right"> <a href="<?= site_url('erp/print-order/').$segment_id;?>" target="_blank" class="collapsed btn btn-sm waves-effect waves-light btn-success m-0">
              ...
              <?php if($user_info['user_type'] != 'customer'){ ?>
				  <?php if($result['status'] == 0) { ?>
                      ...
                      <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-invoice_id="<?php echo $segment_id; ?>" data-status="invoice_pay" data-target=".view-modal-data">
                      <?= lang('Invoices.xin_pay_invoice');?>
                      </button>
                      <?php } ?>
                      <?php if(in_array('invoice4',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                      <a href="<?= site_url('erp/edit-order/').$segment_id;?>" class="collapsed btn btn-sm waves-effect waves-light btn-primary m-0">
                      <?= lang('Main.xin_edit');?>
                      </a>
                      <?php } ?>
                  <?php } ?>
                  <?php if($result['status'] == 1) { ?>
                  <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-invoice_id="<?php echo $segment_id; ?>" data-status="packed" data-target=".view-modal-data">
                      <?= lang('Inventory.xin_packing_status');?>
                      </button>
                  <?php } ?>
                  <?php if($result['status'] == 3) { ?>
                  <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-invoice_id="<?php echo $segment_id; ?>" data-status="delivery" data-target=".view-modal-data">
                      <?= lang('Inventory.xin_delivery_status');?>
                      </button>
                  <?php } ?>
              <?php } ?>
            </div>
                  ...
                  <table class="table invoice-detail-table">
                    ...
                    <tbody>
                      <?php foreach($invoice_items as $item){
                      	$product_info = $ProductsModel->where('product_id',$item['item_id'])->first();
					  	if($product_info){
